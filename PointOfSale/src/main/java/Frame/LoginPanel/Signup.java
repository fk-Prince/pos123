...
public class Signup extends JPanel {
    ...
    public Signup(PanelSwitching panelSwitching) {
    ...
    private void initComponents() {
        ...
        MyButton button = new MyButton("Signup", true);
        ...
        TextButton button1 = new TextButton("Already have an account?", " Login");
        ...
        button1.setPreferredSize(new Dimension(185, 20));
        gbc.gridy = 3;
        gbc.insets = new Insets(10, 10, 10, 10);
    ...
    private boolean isInputsValid(FieldPanel username, PasswordPanel password) {
        ...
        Optional<User> user = new UserRepository().checkUser(username.getText());
        if (user.isPresent()) {
            AnimateMessage.showMessage("Duplicate Username", true, migLayout, this, 32, 0);
        } else {
            new UserRepository().addUser(new User(username.getText(), password.getPassword()));
            AnimateMessage.showMessage("Signup Successfully", false, migLayout, this, 32, 0);
        }
    ...
    protected void paintComponent(Graphics g) {
        ...
        g2d.drawString("Signup", (getWidth() - fm.stringWidth("Signup")) / 2, 75);
